...
model.load_weights("./YOLOV3/yolov3")

with open(cfg.TEST.ANNOT_PATH, 'r') as annotation_file:
    for num, line in enumerate(annotation_file):
        annotation = line.strip().split()
        image_path = annotation[0]
        original_boxes = annotation[1:]
        ...
        if cfg.TEST.DECTECTED_IMAGE_PATH is not None:
            image = utils.draw_bbox(image, image_name,bboxes,original_boxes,crop=True,draw_original=True)
            # cv2.imwrite(cfg.TEST.DECTECTED_IMAGE_PATH+image_name, image)
            cv2.imencode('.jpg', image)[1].tofile(cfg.TEST.DECTECTED_IMAGE_PATH+image_name)
